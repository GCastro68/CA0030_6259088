...
/* ----------------------------------------------------------------
  ...
  Historial de Modificaciones : Proved in an external count 01Sep25
  Autor               : 
  Fecha Modificacion : 
  Descripcion        :
*/
...
void CDlgCapturarAbono::imprimirReciboTermica()
{
    ...
    iRenglon += 22; termica->ponerTermica(iRenglon, 50, "Usted Deb�a:");
    ...
    if (bBanderaTienePlan && !bTieneVencidoPlan)
    {
		...
		if(sprintf_s(cTexto, "Afiliado a Protecci�n Salud con: Plan %s $%s", sTexto, cRespuesta) < 0){;}
    ...
    else
    {
        if (iAplicoDescuento == 1)
        {
			if(sprintf_s(cRespuesta, "Descuento Plan ponte al d�a %d%s", prc_descuento, "%") < 0){;}
            iRenglon += 16;
            termica->ponerTermica(iRenglon, 50, cRespuesta);
        }
...
void CDlgCapturarAbono::terminarReciboTermica()
{
    ...
    // IMPRIME MENSAJE DE PROMOCI�N *COPPEL ( EN TERMICA )
    verificarPromocionAsteriscoCoppel();

    // IMPRIME MENSAJE DE PROMOCI�N DIRECTA ( EN TERMICA )
    if (bPromocion)
    {
...
void CDlgCapturarAbono::imprimirEncabezadoTermica()
{
    ...
    termica->ponerTermica(96, 270, "Rep�blica Pte. 2855 Recursos Hidr�ulicos.");
    termica->ponerTermica(116, 110, "Culiac�n Rosales, Sinaloa 80105 RFC: COP-920428-Q20");
    termica->ponerTermica(134, 97, "R�gimen Fiscal: R�gimen General de Ley Personas Morales");
...
void CDlgCapturarAbono::determinaMensajeReciboTermica(char *cMensaje, int iFlagNoDebe)
{
    ...
    if (m_grid.lCliente > 0L)
    {
        ...
        case 'A':
            if (iFlagVencido == 1)
            {
				if(sprintf_s(cMensaje, 100, "Contin�e Pagando Puntualmente. No deje Pagos Vencidos") < 0){;}
            }
            else
            {
                if (iFlagNoDebe == 0)
                {
					if(sprintf_s(cMensaje, 100, "Sea Puntual y Aproveche su Cr�dito") < 0){;}
                }
                else
                {
        ...
        case 'B':
            ...
            {
				if(sprintf_s(cMensaje, 100, "Contin�e Pagando Puntualmente") < 0){;}
            }
            break;
        case 'C':
            if (iFlagVencido == 1)
        ...
        case 'Z':
            ...
            {
				if(sprintf_s(cMensajeClienteZ, "Saldo sujeto a cambios de acuerdo con el aumento al salario m�nimo del pa�s.") < 0){;}
            }
            break;
        default:
            if (iFlagVencido == 1)
...
Peticion: 30165
Descripci�n: Se modifica para cambiar la forma en que se muestra la fecha en la que se va relizar el pago de los convenios.
--------------------------------------------------------------------*/
    ...
    for (iContCtas = iCtasAImp; iContCtas < iTotalCuentas; iContCtas++)
    {
        ...
        case 6: //USTED DEBE HOY
            ...
            else if (memcmp(cDescripcion, "AUTO PROT.", 10) == 0)
            {
                ...
                {
                    termica->ponerTermica(iRen, iCol, "B�sico");
                }
			}
			else if (strncmp(cDescripcion, "SEG. MOTOS", 10) == 0){
        ...
        case 8: //SALDA CON
            ...
            {
				iCol += 5;
				termica->ponerTermica(iRen, iCol, "d�a");
				iCol += LONGITUD_COLUMNA_CTAS;
            }
            break;
        case 9: //HASTA EL DIA...
        ...
        case 10: //Bonificaci�n
            if (lBonificacionX > 0 && lSuPagoX >= lSaldaConX)
            {
                ...
                if (lBonificacionX > 0)
                {
                    ...
                    termica->alinearTexto(IZQUIERDA); termica->ponerTermica(iRen, 50, "Bonificaci�n");
                    termica->alinearTexto(DERECHA);
...
void CDlgCapturarAbono::imprimirEncabezadoConvenioUnico()
{
    ...
    termica->ponerTermica(iRenglon, 250, "Coppel S.A. de C.V. Rep�blica 2855");
    iRenglon += 30;
    termica->ponerTermica(iRenglon, 250, "Culiac�n, Sin. RFC: COP-920428-Q20");
    iRenglon += 50;

    termica->tipoLetra(&cNewFont, "Arial", &pcOldFont, 11);
	if(sprintf_s(cMensaje, "Convenio �nico") < 0){;}
...
void CDlgCapturarAbono::imprimirReciboConvenioTienda(long lTotalImporteConvenio, char *cFecha)
{
    ...
    termica->ponerTermica(iRenglon, 0, cMensaje);
    iRenglon += 25;
	if(sprintf_s(cMensaje, "Antes del d�a %s en la Tienda Coppel.", cFecha) < 0){;}
    termica->ponerTermica(iRenglon, 0, cMensaje);
...
void CDlgCapturarAbono::imprimirReciboConvenioCliente(long lTotalImporteConvenio, char *cFecha)
{
    ...
    iRenglon += 25;
    usingx("###,###", cImporteConvenio, (double)lTotalImporteConvenio);
	if(sprintf_s(cMensaje, "me comprometo a pagar $ %s antes del d�a %s", cImporteConvenio, cFecha) < 0){;}
    termica->ponerTermica(iRenglon, 0, cMensaje);
    iRenglon += 25;
    ...
    iRenglon += 25;
    termica->ponerTermica(iRenglon, 0, "que recoja los art�culos suficientes para cubrir mi deuda.");
    iRenglon += 50;

    termica->ponerTermica(iRenglon, 0, "El cumplimiento de este convenio, le permitir� benefecios");
    iRenglon += 25;
    termica->ponerTermica(iRenglon, 0, "como: Conservar su cr�dito, m�s facilidades al comprar,");
    iRenglon += 25;
...
void CDlgCapturarAbono::imprimeMensajePromocionDirecta()
{
        ...
        (strnlen(cParrafo3, sizeof(cParrafo3)) != 0) && (strnlen(cParrafo4, sizeof(cParrafo4)) != 0))
    {
        ...
        termica->tipoLetra(&cNewFont, "Arial", &pcOldFont, 7);
        iRenglon += 30;
        termica->ponerTermica(iRenglon, 250, "Republica Pte. 2855 Recursos Hidr�ulicos");
        iRenglon += 30;
        termica->ponerTermica(iRenglon, 250, "Culiac�n Rosales, Sinaloa 80105 ");
        iRenglon += 30;
